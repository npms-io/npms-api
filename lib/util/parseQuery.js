...
const keywords = [
    'author', 'maintainer', 'keywords', 'exclude', 'only', 'boost-exact',
...
const searchParamsSchema = Joi.object({
    ...
    exclude: Joi.array().items(Joi.string().trim().valid(flags)).single(),
    only: Joi.array().items(Joi.string().trim().valid(flags)).single(),
...
function ensureUnique(params) {
    ...
    if (params.exclude) {
        params.exclude = uniq(params.exclude);
    }
}
