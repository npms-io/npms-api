...
function fetchFromCouchdb(names, npmsNano) {
    ...
    .map((row) => {
        ...
        if (row.error === 'not_found' || !row.doc) {
            return null;
        }
...
module.exports = (router, npmsNano, esClient) => {
     ...
     * @apiSuccess {object} [error]     The error associated with the last analyze attempt
     *
     ...
     * @apiSuccess (Each info has) {object} [error]     The error associated with the last analyze attempt
     *
     * @apiSuccessExample {json} Response
     *     {
     *       "cross-spawn": {
