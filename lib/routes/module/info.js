...
module.exports = (router, npmsNano, esClient) => {
    router.get('/module/:name', function * () {
        const names = validateNames([this.params.name]);
    ...
    router.post('/module/fetch', bodyParser({ enableTypes: ['json'] }), function * () {
        const names = validateNames(joiValidate(fetchSchema, this.request.body));
