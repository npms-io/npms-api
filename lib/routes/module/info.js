...
module.exports = (router, npmsNano, esClient) => {
     ...
     *     	    -d '["cross-spawn", "react"]'
     */
    const mgetSchema = Joi.array().label('names').min(1).max(250).unique().items(Joi.string()).required();

    router.post('/module/mget',
        bodyParser({ enableTypes: ['json'] }),
        function * (next) {
            this.validated = validateNames(joiValidate(mgetSchema, this.request.body));
            yield next;
        },
        function * () {
            const names = this.validated;
