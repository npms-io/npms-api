...
module.exports = (router, npmsNano, esClient) => {
    /**
     * @api {get} /info/:name Get module info
     * @apiName get
     * @apiGroup info
     *
     ...
     *     curl "https://api.npms.io/info/react"
     */
    router.get('/module/:name',
        function * (next) {
            this.validated = validateNames([this.params.name]);
            yield next;
        },
        function * () {
    ...
    /**
     * @api {post} /info/mget Get several modules info
     * @apiName mget
     * @apiGroup info
     *
     ...
     *       },
     *       "react": {
     *         "module": { "name": "react", "version": "15.0.1", ... },
     *         "evaluation": { "quality": { ... }, "popularity": { ... }, ... },
     *         "score": { "final": 0.95, "detail": { "quality": 0.99, "popularity": 0.97, ... } }
     *       }
     *     }
     *
     * @apiExample {curl} Example usage:
     ...
     *     	    -d '["cross-spawn", "react"]'
     */
    const fetchSchema = Joi.array().label('names').min(1).max(250).unique().items(Joi.string()).required();

    router.post('/module/mget',
        bodyParser({ enableTypes: ['json'] }),
        function * (next) {
            this.validated = validateNames(joiValidate(fetchSchema, this.request.body));
            yield next;
        },
        function * () {
            const names = this.validated;
