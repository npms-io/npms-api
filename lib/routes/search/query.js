...
function buildFilterQuery(params) {
    ...
    if (params.author) {
        const author = mimicRawAnalyzer(params.author);  // Term query, we must "mimic" the raw analyzer

        filters.must.push({
            or: [
                { term: { 'package.author.name': author } },
                { term: { 'package.author.username': author } },
                { term: { 'package.author.email': author } },
            ],
        });
    }

    if (params.maintainer) {
        const maintainer = mimicRawAnalyzer(params.maintainer);  // Term query, we must "mimic" the raw analyzer

        filters.must.push({
            or: [
                { term: { 'package.maintainers.username': maintainer } },
                { term: { 'package.maintainers.email': maintainer } },
            ],
        });
    }

    if (params.keywords) {
        const keywords = params.keywords.map(mimicRawAnalyzer);  // Term query, we must "mimic" the raw analyzer

        filters.must.push({
            terms: { 'package.keywords.raw': keywords },  // Operator is OR
        });
    }
