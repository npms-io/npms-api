...
function buildFilterQuery(params) {
    ...
    if (params.only) {
        params.only.forEach((only) => {
            filters.push({
                term: { flags: only },  // Operator is AND
            });
        });
    }
    ...
    if (params.exclude) {
        filters.push({
            not: {
                terms: { flags: params.exclude },  // Operator is OR
            },
        });
    }
        ...
        body: {
            ...
            query: {
                function_score: {
                    ...
                    query: {
                        bool: {
                            must: filterQuery ? [filterQuery].concat(matchQueries) : matchQueries,
                        },
                    },
...
module.exports = (router, esClient) => {
     ...
     * - `only:deprecated`: Exclude deprecated packages from the results (unstable and insecure are available too)
